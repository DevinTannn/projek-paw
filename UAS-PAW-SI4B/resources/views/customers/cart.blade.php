            ...
            @foreach($menus as $menu)
                <div class="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Rp {{ number_format($menu->price, 0, ',', '.') }} x {{ $cart[$menu->id] }}</p>
                    </div>
                    <p class="font-bold text-orange-600 text-sm">
                        Rp {{ number_format($menu->price * $cart[$menu->id], 0, ',', '.') }}
                    </p>
                </div>
            @endforeach
        </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="number" 
                           ...
                           class="w-full border border-slate-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 rounded-lg p-2 mt-1 outline-none transition-all">
                </div>
                
                <div>
                    <label class="block text-sm font-semibold text-slate-700">Metode Pembayaran</label>
                    <select name="metode_bayar" class="w-full border border-slate-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 rounded-lg p-2 mt-1 outline-none transition-all bg-white">
                ...
                <div>
                    ...
                    <textarea name="catatan" 
                              placeholder="Contoh: Pedas, tidak pakai seledri..."
                              class="w-full border border-slate-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 rounded-lg p-2 mt-1 h-20 outline-none transition-all resize-none"></textarea>
                </div>
            </div>
            
            <div class="flex space-x-3 mt-6">
                <button type="button" onclick="closeModal()" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 py-2.5 rounded-lg font-bold transition-all">
                    Batal
                </button>
                <button type="submit" class="w-full bg-orange-600 hover:bg-orange-700 text-white py-2.5 rounded-lg font-bold transition-all shadow-md">
                    Kirim Pesanan
                </button>
            </div>
        </form>
...
<script>
    ...
    // Penutup Modal otomatis saat user mengklik area background gelap di luar kotak modal
